...
import { Clock, Swords, Trophy, Users, Flame } from "lucide-react";
...
export function BattleHero() {
  ...
  return (
    <div className="relative mb-16 pt-20 lg:pt-32">
      {/* Background Elements */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[400px] bg-orange-500/10 blur-[120px] rounded-full pointer-events-none opacity-50" />
      ...
      <div className="relative z-10 flex flex-col items-center text-center mb-16 max-w-3xl mx-auto px-4">
        <motion.div
          ...
          className="group relative inline-flex items-center gap-2 px-6 py-2 rounded-full bg-white/[0.03] border border-white/10 hover:border-orange-500/50 transition-colors duration-500 mb-8"
        >
          <div className="absolute inset-0 rounded-full bg-gradient-to-r from-orange-500/20 to-red-500/20 opacity-0 group-hover:opacity-100 blur-xl transition-opacity duration-500" />
        ...
        >
          ...
          top of the leaderboard in our high-stakes arena.
        </motion.p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-7xl mx-auto px-4">
        <StatCard
          ...
          value={show(String(live?.battles ?? 0))}
          tag="LIVE"
        />
        <StatCard
          delay={0.4}
          ...
          label="Staked in Battles"
          value={show(formatSol(live?.battleVolumeSol ?? 0))}
          tag="+24H"
        />
        <StatCard
          delay={0.5}

...
// Anchor encodes a Rust enum variant as a single-key object. The precise
// IdlTypes.Side union is only generated by `anchor build` (target/types/), so we
// type the variant explicitly here rather than reaching for `any`.
...
describe("quantum_wager", () => {
  ...
  it("places a YES bet", async () => {
    ...
    await program.methods
      .placeBet({ yes: {} } as Side, new anchor.BN(0.1 * anchor.web3.LAMPORTS_PER_SOL))
      .accounts({
